...
O resultado é o mesmo obtido anteriormente. A função `pnorm()` calcula a probabilidade acumulada à esquerda do quantil fornecido. Fornecemos então o argumento `lower.tail = FALSE` para inverter a calda, já que queremos a probabilidade do peso ser maior que 182 cm.


Assumir uma distribuição normal nos permitiu calcular uma probabilidade teórica. Se quisermos calcular a probabilidade empiricamente, simplesmente precisamos determinar quantas observações ficam acima de 182 e dividir esse número pelo tamanho total da amostra:


```{r}
sum(fdims$hgt > 182) / nrow(fdims)
```


Embora as probabilidades não sejam exatamente as mesmas, elas são razoavelmente próximas. Quanto mais próxima a sua distribuição estiver da normal, mais precisas serão as probabilidades teóricas.


> Exercício 6: Escreva duas questões de probabilidade que você gostaria de responder; um em relação às alturas femininas e outro em relação aos pesos femininos. Calcule essas probabilidades usando tanto a distribuição normal teórica quanto a distribuição empírica (quatro probabilidades ao todo). Qual variável, altura ou peso, apresentou maior concordância entre os dois métodos?

Assumindo a distribuição normal dos pesos e das alturas femininas na população geral, vamos calcular as probabilidades do peso de uma mulher selecionada ao acaso ser menor que 50 e da altura ser menor que 160 e comarar com os respectivos valores empíricos:
...
data.frame(
  teorico = 
    c(pnorm(q = 160, mean = fhgtmean, sd = fhgtsd), pnorm(q = 50, mean = fwgtmean, sd = fwgtsd)),
  empirico = 
    c(sum(fdims$hgt < 160) / nrow(fdims), sum(fdims$wgt < 50) / nrow(fdims)),
  row.names = c("Altura", "Peso")
) %>%
  mutate(
    diferenca = teorico - empirico
  ) %>%
  kable() %>%
  kable_styling(
    full_width = F, 
...
Como podemos ver, a diferença nominal é maior para a diferença entre as distribuições das alturas teórica e empírica.
...
Podemos resumir os critérios de diagnóstico da seguinte maneira:


a. Os pontos no gráfico resíduos versus valores ajustados são dispersos aleatoriamente, sem nenhum padrão específico.

b. Os pontos no gráfico Q–Q normal estão mais ou menos na linha, indicando que os resíduos seguem uma distribuição normal.

c. Tanto no gráfico Escala-Localização quanto no gráfico Resíduos versus Alavancagem, os pontos estão em um grupo, com nenhum ponto muito longe do centro.
...
Como podemos ver, no modelo completo, 10 variáveis estão sendo usadas para prever `wins`. No modelo final selecionado pelo critério AIC, por outro lado, restaram 5 variáveis, as quais estão mostradas na saída do comando `summary()`, logo acima.

2. Construa um intervalo de confiança de 95% para uma das inclinações no modelo final da parte 1.

Para fazer isso, usaremos a função `confint()`, nativa do R, passando como argumento o modelo obtido por meio da seleção reversa com critério AIC.
...
Como resultado, vemos os intervalos de confiança para cada um dos parâmetros que estão sendo estimados no modelo, sendo mostrados os valores extremos do intervalo e os quantis equivalentes na t de Student.
...
Vemos então que os modelos selecionados através dos critérios AIC e $R^2_{adj}$ concordam entre si, e ambos diferem do modelo selecionado usando o critério do p-valor da ANOVA.

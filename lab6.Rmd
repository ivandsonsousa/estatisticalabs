...
Conforme podemos ver na saída dos comandos acima, a média populacional das áreas dos imóveis na cidade de Ames e a média da amostra aleatória simples de 60 casas que retiramos do conjunto original são muito parecidas. De fato, elas diferem por apenas `r round(100*abs( as.double(colMeans(select(ames, area))) - as.double(colMeans(select(amostra, area))) ) / as.double(colMeans(select(ames, area))), digits = 2)`%. 
...
* Utilizar estas estatísticas para calcular um intervalo de confiança.

* Repetir estas etapas 50 vezes.
...
```{r}
ic_dados = data.frame(ic_id = c(1:50, 1:50), ic_limites = c(ic$inf, ic$sup), captura_mu = c(ic$captura_mu, ic$captura_mu))
```
...
ic2 = rdply(50, slice_sample(ames, n = n, replace = T), .id = "Amostra") %>%
  group_by(Amostra) %>%
  summarise(inf = mean(area) - z_gamma2*sd(area)/sqrt(n), 
